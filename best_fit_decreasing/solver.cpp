...
//Naive VM selection policy - each requests is mapped onto the first VM type separately
    vector<VM>
    BestFitDecreasing::vm_generator(vector<Container> requests, const vector<Host> &hosts, const vector<VM> &vm_types) {
        vector<VM> res;
...
//Basic first fit algo - maps each VM onto first available Host, throws if it is impossible
    void BestFitDecreasing::vm_placement(std::vector<VM> vms, std::vector<Host> &hosts) {
