#include <algorithm>
#include <first_fit_decreasing.hpp>
#include <iostream>
...
vector<VM> FirstFitDecreasingSolver::vm_generator(vector<Container> requests, const vector<Host> &hosts,
...
void FirstFitDecreasingSolver::vm_placement(vector<VM> &vms, std::vector<Host> &hosts) {
    ...
    for (auto &current : vms) {
        ...
        if (current.host < 0) throw std::invalid_argument("First fit decreasing cannot fit VMs onto PMs");
    }
}
