...
namespace solvers {
    ...
    void ModifiedSolver::vm_generate_single(std::vector<Container> requests, std::vector<VM>& res,
        ...
        for (pos = 0; pos < vm_min_cnt; ++pos) {
            ...
            int ind = 0;
            while (ind + 1 < medium_vms.size() && medium_vms[ind + 1].cpu_request + res[start + pos].cpu_request <= k_resource_limit &&
        ...
        for (pos = vm_min_cnt - 1; pos > -1; --pos) {
            ...
            if (small_vms.front().cpu_request + small_vms[2].cpu_request + res[start + pos].cpu_request > k_resource_limit ||
                        small_vms.front().memory_request + small_vms[2].memory_request + res[start + pos].memory_request > k_resource_limit) {
                continue;
            }
        ...
        for (pos = 0; pos < vm_min_cnt; ++pos) {
            while (true) {
                ...
                while (ind - 1 >= 0 && sorted_container[ind - 1].cpu_request + res[start + pos].cpu_request <= k_resource_limit &&
