...
std::vector<VM> HarmonicSolver::vm_generator(std::vector<Container> requests, const std::vector<Host>& hosts,
...
void HarmonicSolver::vm_generate_single(std::vector<Container> requests, std::vector<VM>& res, double max_cpu_host,
    ...
    vm_min_cnt = std::max(
        vm_min_cnt,
        static_cast<int>(total_cpu_request / (max_cpu_host / k_vm_min_count - vm_types[0].cpu_overhead) * 1 + 1));
    ...
    vm_min_cnt =
        std::max(vm_min_cnt,
                 static_cast<int>(
                     total_memory_request / (max_memory_host / k_vm_min_count - vm_types[0].memory_overhead) * 1 + 1));
