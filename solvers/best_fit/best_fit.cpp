...
vector<VM> BestFit::vm_generator(vector<Container> requests, const vector<Host> &hosts, const vector<VM> &vm_types) {
    vector<VM> res;
...
void BestFit::vm_placement(vector<VM> &vms, std::vector<Host> &hosts) {
    for (auto &current : vms) {
        ...
        if (ans_ind < 0) throw std::invalid_argument("Best fit cannot fit VMs onto PMs");
        current.host = ans_ind;
        hosts[ans_ind].Place(current);
    }
}

long double BestFit::energy_consumption(const std::vector<Host> &hosts) {
    long double ans = 0;
